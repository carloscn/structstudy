...
static int32_t bt(int64_t (*array)[4], int32_t x, int32_t y, int32_t len, int32_t depth, int64_t *e, int32_t e_len, int32_t find_len)
{
    int32_t i = 0;
    int32_t ret = 0;
    int64_t tmp = 0;
...
static int32_t is_find_path(int64_t (*array)[4], int64_t *dest, int32_t dest_len)
{
    int32_t array_len = 4;
    ...
    for (i = 0; i < array_len; i ++) {
        for (j = 0; j < array_depth; j ++) {
            if (bt(array, i, j, array_len, array_depth, dest, dest_len, 0) == 0) {
                return 0;
            }
        }
    }
...
int main(void)
{
    ...
    if (is_find_path(test_array, input_array, 4) == 0) {
        goto err;
    }
